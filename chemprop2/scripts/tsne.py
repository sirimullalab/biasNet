...
shuffle(color_list)
sizes_list = [1 for _ in range(len(color_list))]
################################################################
...
#    sizes: List[float] = [1, 1, 1, 1, 1]  # Sizes of the points associated with each molecule
    colors: List[str] = color_list  # Colors of the points associated with each dataset
    sizes: List[float] = sizes_list
    scale: int = 1  # Scale of figure
    plot_molecules: bool = False  # Whether to plot images of molecules instead of points
    max_per_dataset: int = 200000  # Maximum number of molecules per dataset; larger datasets will be subsampled to this size
...
def compare_datasets_tsne(args: Args):
    ...
    morgans = [] # added GK
    for smile in tqdm(smiles, total=len(smiles)): # added by GK
        try:
            morgans.append(morgan_generator(smile)) # ""
        except:
            pass
    #morgans = [morgan_generator(smile) for smile in tqdm(smiles, total=len(smiles))] # commented by GK

    print('Running t-SNE')
    start = time.time()
    tsne = TSNE(n_components=2, init='pca', random_state=0, metric='jaccard',n_jobs=-1)
    ...
    for slc, color, label, size in zip(slices, args.colors, labels, args.sizes):
        if args.plot_molecules:
            ...
            # Plots points
            plt.scatter(X[slc, 0], X[slc, 1], s=300 * size, color=color, label=label)
...
"""
    Example command -
python tsne.py --smiles_path ../subcell_project/tsne_test/*.csv --save_path ../subcell_project/tsne_results/tsne_.png --smiles_column Canonical_Smiles
"""
