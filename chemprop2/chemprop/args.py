...
class TrainArgs(CommonArgs):
    ...
    metric: Literal['auc', 'prc-auc', 'rmse', 'mae', 'mse', 'r2', 'accuracy', 'cross_entropy', 'class_report', 'cohen_k', 'f1'] = None  # Metric to use during evaluation. Defaults to "auc" for classification and "rmse" for regression.
    ...
    def process_args(self) -> None:
        ...
        if not ((self.dataset_type == 'classification' and self.metric in ['auc', 'prc-auc', 'accuracy', 'class_report', 'cohen_k', 'f1']) or
                (self.dataset_type == 'regression' and self.metric in ['rmse', 'mae', 'mse', 'r2']) or
                (self.dataset_type == 'multiclass' and self.metric in ['cross_entropy', 'f1', 'accuracy', 'class_report', 'cohen_k'])):
